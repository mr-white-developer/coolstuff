...
public class CategoryDao {
	...
	public ViewResult<Category> update(Category data,Connection conn){
		...
		try{
			...
			ps.add("mdate", CommonUtility.convertDate_2db(data.getMdate()), PrepareQuery.Operator.EQUAL, PrepareQuery.Type.DATE);
	...
	public ViewResult<Category> getAll(Category cri,Connection conn) {
		...
		try{
			String sql = "select row_number() over(order by cat.name asc) as row_num,"
					+ "count(*) over() as total,"
					+ "cat.id as cat_id,"
					+ "cat.code as cat_code,"
					+ "cat.name as cat_name,"
					+ "cat.status as cat_status,"
					+ "cat.cdate as cat_cdate,"
					+ "cat.mdate as cat_mdate"
			...
			if(cri.getSubCategories().size() == 1){
				ps.add("sub.name", cri.getSubCategories().get(0).getName(), PrepareQuery.Operator.LIKE_ALL,PrepareQuery.Type.VARCHAR);
				ps.add("sub.code", cri.getSubCategories().get(0).getCode(), PrepareQuery.Operator.LIKE_ALL,PrepareQuery.Type.VARCHAR);
			}
			
			sql += ps.createWhereStatement(cri.getCurrentRow(), cri.getMaxRowsPerPage(),
					"cat.id,cat.code,cat.name,cat.status,cat.cdate,cat.mdate"
					, "cat.name", "asc");
			PreparedStatement stmt = conn.prepareStatement(sql);
